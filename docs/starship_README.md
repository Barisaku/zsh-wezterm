...
## タブを大量に開いた時の scan timeout

Starship はプロンプト表示時に、現在のディレクトリを見て Git / 言語 runtime / 設定ファイルの有無を判定します。
タブを一気にたくさん開くと同時に scan が走り、次の警告が出ることがあります。

```text
[WARN] - (starship::context): Scanning current directory timed out.
```

この設定では、警告が出にくく、かつ待ちすぎない値として次にしています。

```toml
scan_timeout = 100
```

まだ出る場合は `150` から `200` くらいまで上げます。
ただし大きくしすぎると、重いディレクトリでプロンプト表示が待たされます。

より軽くしたい場合は、使っていない runtime / cloud module を無効化してください。
